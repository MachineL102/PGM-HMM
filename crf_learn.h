...
bool open_triandata_task_and_buildFeature(Sentence* sentence, std::set<std::string>* labels,
...
int crf_learn(int argc, char** argv) {
    ...
    // 由于某个线程完成对某个sentence的open_traindata_task任务之后即可buildFeature，所以这里将两个方法合并，避免线程空闲
    for (auto s_ptr : x) {
        // 设置 answer和lines  然后设置feature_index
        future_bool.emplace_back(tp.commit(open_triandata_task_and_buildFeature, s_ptr,
            &labels, unigram_templs, bigram_templs));
    }
    x[0]->plot();
    ...
    while (i++ < 100) {
        for (Sentence* s_ptr : x) {
            // 此处可以看到大量的并行任务，在CRF++中每次都要创建并结束一个新的线程，显然不够合适
